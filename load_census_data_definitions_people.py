...
for row in ws.rows:
    ...
    if ns > nskip:
        ...
        if row_array[0] != None:
            desc = row_array[0]
        row_array[0] = desc
        if len(row_array[2]) > maxlen:
            maxlen = len(row_array[2])
        print(row_array)
        #sql = "INSERT INTO census_2018_persona_definitions VALUES (\"%s\", %d, \"%s\")" %  (row_array[0], int(row_array[1]), row_array[2])
